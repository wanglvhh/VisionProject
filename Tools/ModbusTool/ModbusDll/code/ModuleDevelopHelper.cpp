	...
	*ppInterface = NULL;
	...
	switch( nInterfaceNo ){
 	case (int)E_TOOL_MODBUS_RTUMASTER :
		...
		break ;
	case (int)E_TOOL_MODBUS_RTUSLAVE :
		{
			CModbusRtuSlaveInterface * pVision = new CModbusRtuSlaveInterface();
			*ppInterface = (void*) ((CModbusRtuSlaveBase*) pVision);
		}
		break ;
	case (int)E_TOOL_MODBUS_TCPMASTER :
		{
			CModbusTcpMasterInterface * pVision = new CModbusTcpMasterInterface();
			*ppInterface = (void*) ((CModbusTcpMasterBase*) pVision);
		}
		break;
	case (int)E_TOOL_MODBUS_TCPSLAVE :
		{
			CModbusTcpSlaveInterface * pVision = new CModbusTcpSlaveInterface();
			*ppInterface = (void*) ((CModbusTcpSlaveBase*) pVision);
		}
		break;
	}
	////
