...
extern "C" __declspec(dllexport) DWORD __stdcall DllCreateInterface(long nInterfaceNo,void** ppInterface)
{
	...
	*ppInterface = NULL;
	...
	////
	switch( nInterfaceNo ){
	case (int)E_TOOL_CAMERA_HALCON :
		{
			CHalconCameraInterface * pVision = new CHalconCameraInterface();
			*ppInterface = (void*) ((CCameraBase*) pVision);
		}
		break;
	case (int)E_TOOL_CAMERA_BASLER :
		{
			CBaslerCameraInterface * pVision = new CBaslerCameraInterface();
			*ppInterface = (void*) ((CCameraBase*) pVision);
		}
	}
	////
