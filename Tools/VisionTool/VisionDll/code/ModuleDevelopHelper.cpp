...
extern "C" __declspec(dllexport) DWORD __stdcall DllCreateInterface(long nInterfaceNo,void** ppInterface)
{
	...
	*ppInterface = NULL;
	...
	case (int)E_TOOL_2DVISION_CAMERA :
		{
			CVision2DCameraInterface * pVision = new CVision2DCameraInterface();
			*ppInterface = (void*) ((CVisionBase*) pVision);
		}
		break;
	case (int)E_TOOL_2DVISION_SHAPEMODEL :
		{
			CVisionShapeModelInterface * pVision = new CVisionShapeModelInterface();
			*ppInterface = (void*) ((CVisionBase*) pVision);
		}
		break;
	case (int)E_TOOL_2DVISION_FITLINE :
		{
			CVisionFitLineInterface * pVision = new CVisionFitLineInterface();
			*ppInterface = (void*) ((CVisionBase*) pVision);
		}
		break;
	case (int)E_TOOL_VISIONSYSTEM_COMMREAD :
		{
			CVisionCommReadInterface *pVision = new CVisionCommReadInterface() ;
			*ppInterface = (void*) ((CVisionBase*) pVision);
		}
		break;
	case (int)E_TOOL_VISIONSYSTEM_COMMWRITE :
		{
			CVisionCommWriteInterface *pVision = new CVisionCommWriteInterface() ;
			*ppInterface = (void*) ((CVisionBase*) pVision);
		}
		break;
	}
