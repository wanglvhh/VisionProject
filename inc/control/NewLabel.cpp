...
BOOL CTextRotator::RotateText(UINT uiAngle)
{
	...
	strcpy(pszText,T2A(m_strText));

	char *psz = strtok(pszText, "\n");

	while (psz)
	{
		arrayText.Add((CString)psz);
		iLineCounter++;
		psz = strtok(NULL, "\n");
	}
...
void CNewLabel::DrawAlignedText(CDC *pDC, LPCSTR lpszText, RECT stRect, UINT uiAlignment)
{
	...
	if (bSingleLine || (!bSingleLine && bIsVTop))
	{
		...
		pDC->DrawText((CString)lpszText, -1, &rect, uiFormat);
...
CString CNewLabel::MakeVerticalText(LPCSTR lpszText)
{
	...
	strSource = (lpszText) ? lpszText : T2A(m_strText);
...
{
	USES_CONVERSION;
	strcpy(W2A(m_stLF.lfFaceName), T2A(szFaceName));
	ReconstructFont();
	return *this;
}
...
CNewLabel &CNewLabel::SetText(LPCTSTR szText)
{
	...
	if (m_bVerticalText)
	{
		SetWindowText( MakeVerticalText(T2A(szText)) );
	}
...
{
	USES_CONVERSION ;
	m_cControlToolTip.SetText(T2A(lpszText));
	return *this;
}
...
CNewLabel &CNewLabel::SetVerticalText(BOOL bSet)
{
	...
	{
		SetWindowText( MakeVerticalText(T2A(m_strText)) );
	}
...
void CNewLabel::OnPaint()
{
	...
	CRect    rectClient;
	CString  strText = (!m_bVerticalText) ? m_strText : MakeVerticalText(T2A(m_strText));
	...
	if (m_hBackImage &&
		m_uiAngle == 0 &&
		(!IsMultiLineString(T2A(strText)) || (IsMultiLineString(T2A(strText)) && GetVertAlignment() == VERT_TOP)))
	{
		dc.SetTextColor(!m_bUseDefaultTextColor ? m_clrTextColor : ::GetSysColor(COLOR_WINDOWTEXT));

		DrawAlignedText(&dc, T2A(strText), rectClient, m_uiAlignment);
	}
	else {
