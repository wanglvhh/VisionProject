...
namespace csv {
Parser::Parser(const std::string &data,const std::vector<std::string> &InHeader,const DataType &type, char sep)
		: _type(type), _sep(sep)
{
	std::string line;
	if (type == eFILE)
...
Parser::Parser(const std::string &data, const DataType &type, char sep)
  ...
  void Parser::setHeader(std::vector<std::string> Inheader)
  {
	  _header.clear() ;
	  _header = Inheader ;
  }

  void  Parser::setOriginalFile(std::vector<std::string> Inheader)
  {
	  _originalFile.clear() ;
	  std::string strOriginal("") ;
	  for (int i = 0 ;i < Inheader.size();i++)
	  {
		  if (i == Inheader.size()-1)
		  {
			  strOriginal += Inheader[i] ;
		  }
		  else
		  {
			  strOriginal += Inheader[i] ;
	  ...
	  _originalFile.push_back(strOriginal) ;
  }
